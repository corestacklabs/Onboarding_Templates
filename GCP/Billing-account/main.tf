...
resource "google_bigquery_data_transfer_config" "query_config_1" {
  ...
  params = {
    query = <<-EOF
      DECLARE bucket_name STRING DEFAULT '${local.bucket_name}';  
      ...
      DECLARE bucket_path STRING DEFAULT CONCAT('gs://', bucket_name, '/', FORMAT_DATE('%G-%m', current_date), '/', FORMAT_DATE('%F', current_date), '/*.snappy.parquet'); 
      EXPORT DATA OPTIONS (uri=(bucket_path), compression='Snappy', format='Parquet', overwrite=True) AS 
      SELECT
      *, (SELECT STRING_AGG(display_name, '/') FROM B.project.ancestors) organization_list 
      FROM `${var.table_id}` as B 
...
resource "google_bigquery_data_transfer_config" "query_config_2" {
  ...
  params = {
    query = <<-EOF
      ...
      DECLARE bucket_path STRING DEFAULT CONCAT('gs://', bucket_name, '/', FORMAT_TIMESTAMP('%G-%m', start_date), '_backfill/*.snappy.parquet'); 
      EXPORT DATA OPTIONS (uri=(bucket_path), compression='Snappy', format='Parquet', overwrite=True) AS 
      SELECT  
      *, (SELECT STRING_AGG(display_name, '/') FROM B.project.ancestors) organization_list 
      FROM `${var.table_id}` as B 
